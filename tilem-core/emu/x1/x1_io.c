...
void x1_z80_out(TilemCalc* calc, dword port, byte value)
{
	...
	case 0x03:
		...
		// Debug logging
		static int port3_write_count = 0;
		if(port3_write_count++ % 100 == 0) {
			fprintf(stderr, "PORT3 write: value=0x%02X onkeyint=%d lcd.active=%d poweronhalt=%d\n",
			        value, calc->keypad.onkeyint, calc->lcd.active, calc->poweronhalt);
			fflush(stderr);
		}
		break;
